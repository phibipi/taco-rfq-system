...
# --- TAB 5: ACCESS RIGHTS (UPDATE: FORMAT VENDOR DROPDOWN) ---
    ...
    # --- HALAMAN 2: MONITORING & SUMMARY ---
       ...
       # BIG MERGE MASTER (Untuk Tab 2, 3, 4)
        ...
        tabs = st.tabs(["⏳ Submit Monitor", "✅ Lock Data", "📊 Summary", "🖨️ Print Dokumen", "📥 SPH Uploads", "Template", "comparison", "Price Search"])
...
# ==============================================================================
        ...
        # === TAB 2: SEARCH VENDOR BY ROUTE ===
        with tabs[7]: # 🎯 TAB BARU ADMIN: PRICE SEARCH WITH HARGA
            st.subheader("🔍 Price Search (Admin Mode)")
            st.caption("Cari vendor berdasarkan rute Tahap 2 lengkap dengan visual Harga Unit.")
            if df_master.empty:
                st.info("Data belum tersedia.")
            else:
                # Filter Utama
