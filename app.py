        ...
        if df_master.empty:
            ...
            if search_dest:
                ...
                if not df_search.empty:
                    ...
                    # === 2. EKSTRAK ROUND DARI id_transaksi (RIGHT 1) ===
                    df_search['round'] = df_search['id_transaksi'].apply(
                        lambda x: x.strip()[-1] if str(x).strip() else '1'
                    )

                    # === 3. DEDUP: KALAU ADA DUPLIKAT, AMBIL YANG ROUND = 2 ===
                    # Sort ascending → keep='last' otomatis ambil round terbesar (2 > 1)
                    ...
                    # === 4. GROUP_ID MATCH: 5 HURUF PERTAMA route_id ===
                    ...
                    if not df_md.empty:
                        ...
                        df_md['id_multidrop'] = df_md['id_multidrop'].astype(str).str.strip()
                        ...
                        # EKSTRAK ROUND MULTIDROP: RIGHT(1) dari id_multidrop
                        df_md['round_md'] = df_md['id_multidrop'].apply(
                            lambda x: x.strip()[-1] if str(x).strip() else '1'
                        )

                        # DEDUP MULTIDROP: kalau duplikat, ambil round_md = 2
                        df_md = df_md.sort_values(
                            by=['vendor_email', 'group_id_md_clean', 'validity', 'round_md'],
                            ascending=True
                        )
                        ...
                        # === 6. LOOKUP: email + 5 huruf route_id + validity + round ===
                        df_result = pd.merge(
                            df_search_clean,
                            md_subset,
                    ...
                    # === 7. SAFETY NET: PASTIKAN TIDAK ADA DUPLIKAT DI HASIL AKHIR ===
